...
- `<TwoLayersDiagram />`, `<TestPyramidDiagram />`, `<HybridFlowDiagram />` are article-scoped inline SVGs for *three ways to test C++ from Python* (the two-layer split, the test pyramid, the Mode-3 topology). No props; same single-accent rule as `<CommitFlowDiagram />` — one sibling component per diagram, labels in text. Diagram labels stay English even in the ES article (same precedent as `<CommitFlowDiagram />`).
