...
| `<TwoLayersDiagram />` | `src/components/mdx/TwoLayersDiagram.astro` | Inline SVG for the *three ways to test C++ from Python* article: Python binds Layer A (pure logic) directly; Layer B (the I/O shell) wraps and drives it. Article-scoped, single-accent slate. |
| `<TestPyramidDiagram />` | `src/components/mdx/TestPyramidDiagram.astro` | Inline SVG test pyramid for the same article — Mode 1 (unit) base, Mode 2 (integration) middle, Mode 3 (e2e) apex, with side labels for what each tests. Article-scoped, single-accent slate. |
| `<HybridFlowDiagram />` | `src/components/mdx/HybridFlowDiagram.astro` | Inline SVG Mode-3 topology for the same article: real C++ client devices + a Python mock on the real wire to the real server, observed by the orchestrator via callbacks. Article-scoped, single-accent slate. |
