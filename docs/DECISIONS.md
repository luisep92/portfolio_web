...
- **Anonymous lineage is allowed without a gate.** Work *extracted* from other projects — a shared skeleton, a generalized module — can be described publicly at the level of shape ("the same spine was built twice, in two applications with nothing in common") without naming anyone, identifying the domain, or shipping any of their code. Naming stays gated on the client's agreement; the anonymous form is not gated, because it carries nothing that identifies them.
- **Anonymity covers the relationship, not just the name.** "Two client applications" names nobody and still overclaims: it implies commissioned, delivered, invoiced work, which is a stronger claim than two private repos still in progress. Describe *what the work was*, not who it was for — the argument almost never depends on the latter. Both bullets first applied in `src/content/projects/{en,es}/saas-template.mdx`.
