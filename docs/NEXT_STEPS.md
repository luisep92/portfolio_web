...
2. **Article share links — done.** [`src/components/ArticleShareLinks.astro`](../src/components/ArticleShareLinks.astro) rendered automatically in the `ArticleLayout` footer (after the body, before the page footer). Four targets: Twitter/X intent, LinkedIn intent, Hacker News submit, and a copy-link button (small inline `<script>` using `navigator.clipboard.writeText`, with a 2s "Copied" feedback). Plain `<a href>` to share intents — no third-party widgets, no tracking. i18n-aware (`share.heading`, `share.copy.label`, `share.copy.success` in `src/i18n/ui.ts`). Component is **not** an MDX component (not used inside MDX bodies; lives at top level of `src/components/`, like `MissingTranslationNotice.astro`). Mastodon/Bluesky deferred until/unless Luis decides they're worth the additional bullets.
