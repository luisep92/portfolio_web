...
### 12. Discoverability — share buttons + OG metadata + Vercel Analytics — done
...
1. **OG metadata audit on `BaseLayout.astro` — done.** All `<head>` metadata centralized in `BaseLayout`: per-entry `og:title`/`og:description` from layout props, `og:image` falls back to `/og-default.png` (a 1200×630 handcrafted card under `public/`), absolute URLs resolved via `Astro.site`. Beyond the original audit checklist also added: `<link rel="canonical">`, `<link rel="alternate" hreflang>` for EN/ES/x-default, `<link rel="icon">` (`favicon.svg`, monogram "LE"), `og:locale`/`og:locale:alternate`, `article:published_time` for articles, `twitter:card="summary_large_image"`. **`twitter:site`/`twitter:creator` intentionally skipped** — a share preview is about the piece, not the author; authorship lives on the site itself for anyone who clicks through (open sub-decision below). See [ARCHITECTURE.md → "SEO and social meta"](ARCHITECTURE.md). Per-entry image generation (one OG card per article via `@vercel/og`) is **deferred** — kept as an open decision below.
2. **Article share links — done.** [`src/components/ArticleShareLinks.astro`](../src/components/ArticleShareLinks.astro) rendered automatically by `ArticleLayout` **at both top (after the description, before the body) and bottom (after the body)** — top instance lets early-engaged readers share without scrolling to the end; bottom instance respects the natural "I finished, I'll share" reflex. Four targets: Twitter/X intent, LinkedIn intent, Hacker News submit (each suffixed with the existing `→` external-link convention from `ProjectLayout`), and a copy-link button (small inline `<script>` using `navigator.clipboard.writeText`, with a 2s "Copied" feedback). Plain `<a href>` to share intents — no third-party widgets, no tracking. i18n-aware (`share.heading`, `share.copy.label`, `share.copy.success` in `src/i18n/ui.ts`). Component uses `aria-label` (not `aria-labelledby`) so the two instances don't collide on duplicate IDs. **Not** an MDX component — lives at top level of `src/components/`, like `MissingTranslationNotice.astro`. Mastodon/Bluesky deferred until/unless Luis decides they're worth the additional bullets.
3. **Vercel Analytics + close `DECISIONS.md` → "Analytics" — done.** [`@vercel/analytics`](https://www.npmjs.com/package/@vercel/analytics) installed (the `/astro` entrypoint); mounted once in [`BaseLayout.astro`](../src/layouts/BaseLayout.astro) with `mode={import.meta.env.PROD ? 'production' : 'development'}` so local dev doesn't pollute the dashboard. No custom event tracking beyond the default pageview — the question for v1 is "is anyone reading?", and pageviews answer it. The "Analytics" entry moved from `Open decisions` to a closed entry in [`DECISIONS.md`](DECISIONS.md) in the same commit with full rationale. Speed Insights deliberately not added here — Step 15 (Lighthouse + a11y) covers performance work.
...
### 12 follow-up — CSP hash for Analytics + positioning fix (README + home anchor) — done

After Step 12 shipped, two issues surfaced that closed in a follow-up session:

- **Vercel Analytics 0/0/0 in the dashboard.** Diagnosed via DevTools Issues panel: the strict CSP from Step 9 (`script-src 'self'`) was blocking the inline registration `<script>` that `<Analytics />` from `@vercel/analytics/astro` injects, so the obfuscated tracking script was never even fetched. Fix: pinned the script's `sha256` in `vercel.json` `script-src`. The hash-rotation procedure (when the `@vercel/analytics` package bumps and the script body changes) is documented in [ARCHITECTURE.md → "Security posture"](ARCHITECTURE.md). **Validation pending until Luis re-tests** post-deploy (filter `script.js` not `insights` — Vercel obfuscates the path); a separate console error about `eval` may need investigation depending on its source.
- **Positioning drift uncovered by a recruiter test.** Luis ran the portfolio through several LLMs as if he were a hiring manager. Several classified him as a frontend developer purely on stack signal (Astro/Tailwind/MDX) — the `README.md` was two lines and the home page had no H1 to anchor a skim, so the visible stack was the only signal a fast reader had. Fix in two parts: (a) `README.md` rewritten as the entry-door of the portfolio with an explicit meta angle — the repo *is* part of the demo: docs-first, decisions written down with their *why*, AI-agent workflows versioned as skills, narrated commits — with a pointer table to `docs/*` and `.claude/skills/`. No defensive "this isn't frontend"; the meta angle is shown by what's versioned. (b) Home page (EN + ES) now opens with `<h1>Luis Escolano</h1>` plus a one-word `Software engineer.` tagline — coherent with the OG default. New strings `home.heading` / `home.tagline` in [`src/i18n/ui.ts`](../src/i18n/ui.ts).

LLM hallucinations of the article body (ChatGPT and Gemini both invented content instead of reading) are an LLM failure mode, out of scope here.
...
Lineage referenced anonymously — the two source repos are private and WIP, so the entry says "two applications with nothing in common" and names neither. It said "two *client* applications" until Luis caught that it sells something it isn't: nobody is named, but it still implies commissioned and delivered work. Both lessons are in [DECISIONS.md → "Public repo from day one"](DECISIONS.md) as two bullets — anonymity covers the relationship, not just the name.

**Ordering: `order: 5`, so it opens the featured list above Aline.** The first card frames the whole page, and the Step 12 follow-up already established that readers bucket Luis fast on thin signal (several LLMs classified him as a frontend dev on stack alone). Opening with a Beat Saber boss fight invites "modder"; opening with a full-stack product invites "ships software", after which Aline in second position reads as range. Aline also still carries a pending writeup, and leading with a stub is expensive. Cost accepted: Aline is the most distinctive entry and loses the top slot. Used `5` rather than renumbering all five, which is what the gaps in [`content-authoring`](../.claude/skills/content-authoring/SKILL.md) exist for.
...
- **Twitter author attribution (`twitter:site`/`twitter:creator`)** — new open sub-decision out of Step 12.1. Currently skipped on principle: a share preview is about the *piece*, not the author. When someone shares a useful article, the implicit framing is "I found this and it's good", not "this is by @LuisEscolano" — adding author attribution funnels traffic to the author's Twitter without the author choosing it. Authorship is already on the destination site for anyone who clicks through. Revisit only if Luis decides he wants the inverse: shares of his content actively promoting his Twitter presence.
