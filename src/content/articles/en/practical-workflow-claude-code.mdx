...
- **Context files** — five markdown files in the repo. Five minutes per session to maintain.
- **The cycle** — eleven short steps. Review at every checkpoint, never skip.
- **Tools beat prompts** — give Claude what to verify with, not what to think about.
...
<AppliedHere>
- [`CLAUDE.md`](https://github.com/luisep92/portfolio_web/blob/main/CLAUDE.md) — the brief for **this very portfolio repo**. Pure router (pointers to docs); zero technical content that could drift.
- [`docs/DECISIONS.md`](https://github.com/luisep92/portfolio_web/blob/main/docs/DECISIONS.md) — every closed architectural decision with its *why*, plus an "Open" section at the bottom for what's still being decided.
- [`docs/NEXT_STEPS.md`](https://github.com/luisep92/portfolio_web/blob/main/docs/NEXT_STEPS.md) — the state-of-the-build, updated at the end of every session. This article landed as Step 11.
- [`.env.example`](https://github.com/luisep92/portfolio_web/blob/main/.env.example) — the entire env contract for the site. Three lines, one comment. The real `.env` is never committed.
</AppliedHere>
...
<AppliedHere>
- [`docs-governance` skill](https://github.com/luisep92/portfolio_web/blob/main/.claude/skills/docs-governance/SKILL.md) — *what fact goes to which doc*, pre-commit checklist, what NOT to put in `CLAUDE.md` or memory. Written after the third time I had to course-correct on doc duplication.
- [`content-authoring` skill](https://github.com/luisep92/portfolio_web/blob/main/.claude/skills/content-authoring/SKILL.md) — slug conventions, frontmatter, MDX components, when to promote a project from `other` to `featured`. Written after authoring this article.
</AppliedHere>
...
<IterationCycle>
  <Step who="you" title="Align state">
    Open the session by making sure context files are current. If `NEXT_STEPS.md` is stale, update it. If the last session introduced new conventions, update `CLAUDE.md`. Claude will read these files; if they're wrong, it starts wrong.
  </Step>
  <Step who="together" title="Define the task">
    Tell Claude what you want to achieve this session. Be specific: *"add the cancel-order endpoint with validation and error handling"*, not *"work on orders"*. If the task is large, ask Claude to break it down first.
  </Step>
  <Step who="claude" title="Plan">
    Claude proposes an approach: which files to touch, what the implementation looks like, what order to follow. For small tasks (one file, one function), this is implicit. For anything touching multiple modules, ask for an explicit plan.
  </Step>
  <Step who="you" title="Review the plan">
    **This is where most mistakes are prevented.** Read what Claude proposes. Does it match the architecture? Does it follow conventions? Is it overcomplicating something? Correct now — not after 300 lines of code.
  </Step>
  <Step who="claude" title="Code">
    Claude writes the implementation. For large tasks, ask it to work in stages — don't let it generate everything in one shot. Stage boundaries are natural review points.
  </Step>
  <Step who="you" title="Review the code">
    Read the generated code. Understand it. If you can't explain what a function does, you can't commit it. This is not optional — it's the core of the security model. Claude generates fast; you validate thoroughly.
  </Step>
  <Step who="you" title="Test">
    Compile and verify the code works. Run existing tests to check nothing is broken. If your project touches services that are sensitive, costly, or that you don't want exposed to an AI agent's environment — payment APIs, paid LLM keys, internal modules you can't share publicly — mock them. Use a `.env` to keep real credentials out of the working directory entirely. Claude doesn't need real Stripe access to verify your code calls it correctly; it needs a stub that responds with the right shape.

    The principle: **isolate the dangerous edges, not the whole environment**. A full sandboxed VM is overkill for most projects; a discipline of mocked services and untracked secrets is enough.
  </Step>
  <Step who="you" title="Visual verification">
    If the change affects output or behavior, verify visually. Don't trust that "it compiles" means "it works". Run the actual scenario end-to-end.
  </Step>
  <Step who="together" title="Document">
    Ask Claude to update `NEXT_STEPS.md` with what was done, decisions made, and what to do next. Review the update — Claude tends to be verbose; trim to what matters.
  </Step>
  <Step who="you" title="Commit & organize">
    Commits must be small, atomic, one concern each. Claude Code advances very fast — if you don't commit frequently, you'll end up with a massive diff that's impossible to review. Organize the commits to tell a story: migration first, then model, then logic, then tests. Each commit should be a reviewable unit with a clear message.
  </Step>
  <Step who="you" title="Push & self-review">
    Push to your code review tool — GitHub, GitLab, Gerrit, whatever you use. Then open the changeset (or pull request, or merge request) in the web UI and review your own code as if you were the external reviewer. Even solo, treat your future self pulling this PR in six months as the external reviewer. Different context catches different things. Approve only when you're confident.
  </Step>
</IterationCycle>
...
<Takeaway label="Why this matters more than prompt engineering">
A well-tooled Claude with a good context file beats a poorly-tooled Claude with the world's most carefully crafted prompt. Prompts compress intent into language; tools let Claude observe reality. **Reality wins.** Invest more time setting up tools than writing perfect prompts — prompts improve naturally as you work; tools compound over every future session.
</Takeaway>
...
<AppliedHere>
- [`docs/NEXT_STEPS.md`](https://github.com/luisep92/portfolio_web/blob/main/docs/NEXT_STEPS.md) — the live handoff doc for this site. Top header is the current state in one paragraph; below it, every step is "what was done" + "validation criteria". You could pick this repo up cold from this file alone.
</AppliedHere>
...
<AppliedHere>
- [Step 11 commit range](https://github.com/luisep92/portfolio_web/compare/86cb7c9~3...86cb7c9) — the three commits above as a diff range; click any commit to see its full message and reviewable diff.
- [Full git history of this site](https://github.com/luisep92/portfolio_web/commits/main) — every step of building this portfolio has been a feature branch + PR with atomic commits. The history reads as the real changelog.
</AppliedHere>
...
<AppliedHere>
- [`.env.example`](https://github.com/luisep92/portfolio_web/blob/main/.env.example) — three lines. The site has no required env vars at v1; the file documents that explicitly so a fresh clone has nothing to set up before running `pnpm dev`.
- [`.gitignore`](https://github.com/luisep92/portfolio_web/blob/main/.gitignore) — `.env` and `.env.*` excluded except `.env.example`. Belt-and-braces: even if someone accidentally creates `.env.production`, git refuses to stage it.
</AppliedHere>
...
<Takeaway label="On accountability">
Claude Code doesn't suffer the consequences of a mistake. You do. *"Claude generated it"* is not an explanation, and it's certainly not an excuse. If your name is on the commit, you own it. The tools in this guide exist to help you catch problems early, not to shift responsibility. **Claude is the copilot. The pilot signs the flight log.**
</Takeaway>
...
<AppliedHere>
- [`Bisbot/src/GptWrapper.py`](https://github.com/luisep92/Bisbot/blob/main/src/GptWrapper.py) — a Discord bot I built where the GPT wrapper sets up exactly that kind of format-as-persona prompt. The *"you are X"* framing earns its place because the output is going into a chat client with hard constraints, not into a code review.
</AppliedHere>

### Cross-repo agents that act unsupervised
Tools that run an agent continuously over your repositories and let it commit, open PRs, or decide what to work on without you in the loop. I don't. Each repo is its own session, and every commit goes through me before it lands on `main`. My git workflow (branch + PR + e2e + preview, see [`docs/DECISIONS.md`](https://github.com/luisep92/portfolio_web/blob/main/docs/DECISIONS.md)) is built around that as a hard constraint, not a checkpoint an automated agent could plausibly bypass.
